...
static void convertGoals(const bio_ik_msgs::IKRequest &ik_request,
    ...
    for (auto &m : ik_request.touch_goals) {
        ik_options.goals.emplace_back(new bio_ik::TouchGoal(m.link_name, p(m.position), p(m.normal), w(m.weight)));
    }
  #else
    ...
    for (auto &m : ik_request.joint_variable_goals) {
        ik_options.goals.emplace_back(new bio_ik::JointVariableGoal(m.variable_name, m.variable_position, w(m.weight), m.secondary));
    }
    ...
    for (auto &m : ik_request.cone_goals) {
        ik_options.goals.emplace_back(new bio_ik::ConeGoal(m.link_name, p(m.position), w(m.position_weight),
...
static bool getBioIK(bio_ik_msgs::GetIK::Request &request,
    ...
    if (success) {
        response.ik_response.error_code.val = moveit_msgs::MoveItErrorCodes::SUCCESS;
    } else {
        response.ik_response.error_code.val = moveit_msgs::MoveItErrorCodes::NO_IK_SOLUTION;
    }
...
int main(int argc, char **argv) {
    ...
    ros::ServiceServer get_position_ik = node_handle.advertiseService("bio_ik/get_position_ik", &getPositionIK);
  
    ros::ServiceServer get_approximate_ik = node_handle.advertiseService("bio_ik/get_approximate_ik", &getApproximateIK);
  
    ros::ServiceServer get_bio_ik = node_handle.advertiseService("bio_ik/get_bio_ik", &getBioIK);
